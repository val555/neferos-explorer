...
export default function SystemPage() {
  ...
  // Sécurité
  if (!foundSystem) return <div className="pt-40 text-center text-white">Système introuvable</div>;
  ...
  return (
    <div className="w-full h-screen overflow-hidden relative flex items-center justify-center">
      ...
      {/* --- LE SYSTÈME SOLAIRE --- */}
      {/* On utilise une échelle fixe ou responsive */}
      <div className="relative w-1506h-150wmd:w-200h-[800px] flex items-center justify-center">
        
        {/* L'ÉTOILE CENTRALE */}
        <div className={`absolute w-24 h-24 md:w-32 md:h-32 rounded-full shadow-[0_0_80px_currentColor] z-10
        ...
        {foundSystem.planets.map((planet, index) => {
          ...
          const orbitDiameter = 300 + (index * 150); // 300px, 450px...
            ...
            >
              ...
              >
                ...
                <Link to={`/planet/${planet.id}`} className="group cursor-pointer">
                  ...
                  <div 
                    className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-6 h-6 md:w-8 md:h-8 rounded-full bg-white shadow-[0_0_20px_white] hover:scale-125 transition-transform duration-300 z-20"
                  >
                    ...
                    <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                      <div className="bg-neferos-bg-dark/90 border border-neferos-accent px-3 py-1 rounded text-center min-w-25">
      ...
      {/* MENU DE NAVIGATION ENTRE SYSTÈMES (Optionnel, en bas à droite) */}
      {/* On pourra ajouter ici la liste des autres systèmes du même secteur plus tard */}
